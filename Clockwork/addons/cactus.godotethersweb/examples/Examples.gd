...
func connect_buttons():
	...
	$ContractRead.connect("pressed", get_zk_proof)
...
func get_zk_proof():
	...
	var inputs = {"a": 4, "b": 3}
	var callback = EthersWeb.create_callback(self, "show_proof")
	EthersWeb.calculateProof(inputs, callback)

func show_proof(callback):
	if has_error(callback):
		return
	
	var proof = callback["result"]
	print_log("PROOF:\n" + str(proof) + "\n\n")
