...
contract CardGame is VRFV2PlusWrapperConsumerBase, ReentrancyGuard {
    ...
    struct game {
        ...
        uint256[TABLE_SIZE] localSeedHashes;
    ...
    // Players must commit to a specific token, specific ante, and specific localSeedHash when buying
    // a Chainlink VRF seed.
    function buyHandSeed(address playerAddress, address gameToken, uint256 ante, uint256 localSeedHash) payable public nonReentrant {
        ...
        if (player.localSeedHash != 0) revert AlreadyRequestedSeed();
        ...
        // Cache the localSeedHash to be checked during the proof.
        player.localSeedHash = localSeedHash;
        player.ante = ante; 
    ...
    // using a Chainlink VRF seed combined with the committed secret seed.
    function proveHand(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[4] calldata _pubSignals) public nonReentrant {
        ...
        uint256 localSeedHash = player.localSeedHash;
        ...
        // The localSeedHash must match the on-chain localSeedHash.
        if (_pubSignals[0] != localSeedHash) revert InvalidLocalSeed();
        
        // Seed used in ZKP must match on-chain VRF seed
        // Apply the modulus first, otherwise bigger values will not validate correctly
        if (_pubSignals[2] != playerVRFSeed % FIELD_MODULUS) revert InvalidVRFSeed();
    ...
    // to draw 2 new cards.  The player also commits a secret local seed
    // using the localSeedHash.
    // Only callable during the first 2 minutes of the game
    function swapCards(address gameToken, uint256 discardedCardsHash, uint256 localSeedHash) public payable {
        ...
        // A Poseidon hash of the secret local seed is committed,
        // to be later validated against the ZKP's public output.
        session.localSeedHashes[playerIndex] = localSeedHash;
    ...
    function proveSwapCards(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[6] calldata _pubSignals) public {
        
        // Check that the player is eligible to prove the swap.
        address gameToken = address(uint160(_pubSignals[5]));
        ...
        uint256 vrfSwapSeed = session.vrfSwapSeeds[playerIndex];
        ...
        if (!IZKPVerifier(swapZKPVerifier).verifySwapProof(_pA, _pB, _pC, _pubSignals)) revert InvalidZKP();
         ...
         // Validate the localSeedHash against the committed hash.
        if (session.localSeedHashes[playerIndex] != _pubSignals[0]) revert InvalidLocalSeed();

        // Validate the discarded cards against the committed hash.
        if (session.discardedCards[playerIndex] != _pubSignals[1]) revert InvalidDiscard();

        // Validate against old hand
        if (player.currentHand != _pubSignals[2]) revert InvalidHash();

        // Validate VRF seed
        // Must apply modulus for large numbers to validate correctly
        if (_pubSignals[4] != vrfSwapSeed % FIELD_MODULUS) revert InvalidVRFSeed();

        // Update to new hand
        player.currentHand = _pubSignals[3];
